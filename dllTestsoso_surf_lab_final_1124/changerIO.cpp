...
CHANGE_RGN* SceneChangeDetector(char* inputPath1,char* inputPath2,char* outputPath,DETECT_PARAM inputParam1,PRESET_POS_INFO inputParam2,bool flag)
{
	...
	if(presetPosInfo.fCarrierHt>1000) presetPosInfo.fCarrierHt=1000;
...
//cout<<"save cf_bw_a ok"<<endl;
   ChangeDetector(rImgA[2],rImgB[2],params,output3,outputCorrelationCf,"/home/tuxiang/liquan/Pictures/cf_bw_b.jpg");
     //finish4=clock();
     //cout<<"ChangeDetector time is "<<(finish4-start4)/1000000<<"s"<<endl;
...
{   
    ...
    //Ptr<FeatureDetector> detector = FeatureDetector::create( "BRISK" );//创建特征检测器 SURF BRISK
    //Ptr<DescriptorExtractor> descriptor_extractor = DescriptorExtractor::create( "BRISK" );//特征向量生成器
  ...
  //clock_t start3,finish3;
   //	start3=clock();
    ...
    detector.detect( img1, kp1 );//检测img1中的SIFT特征点，存储到m_LeftKey中
    ...
    matcher.radiusMatch(descriptors2,descriptors1,matches,120.0);

     //RANSAC匹配过程
    Mat p1(matches.size(), 2, CV_32F);
    Mat p2(matches.size(), 2, CV_32F);

    int num = 0, matchesnum = 0;
    char flag = 0;
    
    vector<vector<DMatch> > goodMatches;

    while(num < matches.size())
    {
        if(matches[num].size() != 0 && matches[num].size() < 2)
	{
	    p1.at<float>(matchesnum, 0) = kp1[matches[num][0].queryIdx].pt.x;
	    p1.at<float>(matchesnum, 1) = kp1[matches[num][0].queryIdx].pt.y;

	    p2.at<float>(matchesnum, 0) = kp2[matches[num][0].trainIdx].pt.x;
	    p2.at<float>(matchesnum, 1) = kp2[matches[num][0].trainIdx].pt.y;
	    matchesnum++;
	    goodMatches.push_back(matches[num]);
	}
	num++;	
    }

     cout<<"matchesnum="<<matchesnum<<endl;
   //矩阵H用以存储RANSAC得到的单应矩阵
    H = findHomography(p1, p2, RANSAC);
    ...
    cout<<"proheight="<<proheight<<endl;
      ...
      if(abs(start1)>1000||abs(start2)>500||prowidth>1920||proheight>1080||prowidth<0||proheight<0||abs(start1)>proheight||abs(start2)>prowidth)
    {
       Hflag=0;
	   cout<<"correction is fail"<<endl;
       return ;
    }
    ...
    if(start1>0&&start2>0)
    {
	      corr_img1=mat1(Range(start1 ,proheight),Range(start2,prowidth));
              corr_img2=mat2(Range(start1 ,proheight),Range(start2,prowidth));
	      origin.x=start1;
	      origin.y=start2;
    }
