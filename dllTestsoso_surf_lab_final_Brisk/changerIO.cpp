...
{   
    ...
    Ptr<FeatureDetector> detector = FeatureDetector::create( "BRISK" );//创建特征检测器 
    Ptr<DescriptorExtractor> descriptor_extractor = DescriptorExtractor::create( "BRISK" );//特征向量生成器
    Ptr<DescriptorMatcher> descriptor_matcher = DescriptorMatcher::create( "BruteForce-Hamming" );//创建特征匹配器
   ...
   // int MAX_KEYPOINTS = 10000;
    ...
    descriptor_matcher->radiusMatch( descriptors1, descriptors2, matches, 100.0 );
    ...
    for(int i=0; i<matches.size(); i++)
    {
	    ...
	    else if (matches[i].size() > 1)
	    {
		    ...
		    {
			    if (matches[i][j].distance < mindis)
			    {
				    mindis = matches[i][j].distance;
				    flag = j;
			    }
		    }
		    goodMatches.push_back(matches[i][flag]);
	    }
    }
    //RANSAC匹配过程
    ...
    int InlinerCount = ptCount - OutlinerCount;   // 计算内点
     ...
     // cout<<"ptCount="<<ptCount<<endl;
     // cout<<"OutlinerCount="<<OutlinerCount<<endl;
   // cout<<"InlinerCount="<<InlinerCount<<endl;
    
     if(InlinerCount==0)
    {
       Hflag=0;
       cout<<"Two images is different"<<endl;
       return ;
    }
  ...
  //  cout<<"findHomography end"<<endl;
   ...
   cout<<"scene_corners[0]"<<scene_corners[0]<<endl;
    ...
    cout<<"proheight="<<proheight<<endl;
     ...
     if(proheight<0||prowidth<0||abs(start1)>100||abs(start2)>100||proheight>1080||prowidth>1920)
    {
       Hflag=0;
